...
<title>set_recordalter.jsp</title>
...
<table width="100%" border="0" align="center" cellspacing="0" cellpadding="2" style="border-collapse: collapse" bordercolor="#C0C0C0">
  <tr>
    ...
    <select name="item" id="item">
      ...
      <c:forEach var="myitem" items="${itemlist}">
      	<option value="${myitem.itemid }" <c:if test="${myitem.itemid eq record.itemid}">selected="selected"</c:if>>${myitem.itemname}</option>
      </c:forEach>
    </select>
    </td>
  </tr>
  <tr>
    ...
    <select name="sex" id="sex">
      <option value="1" <c:if test="${record.sex eq 1}">selected="selected"</c:if>>男</option>
      <option value="0" <c:if test="${record.sex eq 0}">selected="selected"</c:if>>女</option>
    </select>
    </td>
  </tr>
  <tr>
    <td align="right">比赛成绩：</td>
    <td><input name="score" type="text" id="score" onblur="checkFormat(this);" value="${record.score}"></td>
  </tr>
  <tr>
    <td align="right">选手姓名：</td>
    <td><input name="playername" type="text" id="playername"  value="${record.playername}"></td>
  </tr>
  <tr>
    <td align="right">系别：</td>
    <td><input name="depart" type="text" id="depart" value="${record.departname}" ></td>
  </tr>
  <tr>
    <td align="right">运动会名称：</td>
    <td><input name="sportsname" type="text" id="sportsname" value="${record.sportsname}"></td>
  </tr>
  <tr>
    <td align="right">破记录时间：</td>
    <td><input readonly="readonly" name="scoretime" class="Wdate" type="text" id="scoretime" onClick="WdatePicker({dateFmt:'yyyy-MM'})" value="${record.recordtime}"></td>
  </tr>
    <tr>
    ...
    <select name="level" id="level">
      ...
      <option value="0" <c:if test="${record.recordlevel eq 0}">selected="selected"</c:if>>院级</option>
      <option value="1" <c:if test="${record.recordlevel eq 1}">selected="selected"</c:if>>省级</option>
    </select>
    </td>
  </tr>
  <tr>
    <td colspan="2" align="center"><input name="" type="button" value="确认修改" onclick="addScore();"/></td>
  </tr>
</table>
</c:forEach>
</body>
